...
def insert_nontree_edge_BFS(u, steps_u, v, steps_v, root, id2node): #inserting a non tree edge
    ...
    if abs(steps_u - steps_v) <= 1:  # no changes to BFS spanning tree
        ...
        q.put(root.val)
        node_in_ST = set()
        node_in_ST.add(root.val)
        while not q.empty():
            new_q = Queue()
            while not q.empty():
                n = q.get()
                for c in id2node[n].children:
                    new_q.put(c)
                    node_in_ST.add(c)

            if v in node_in_ST or u in node_in_ST:
                break
            q = new_q
        '''
...
def insert_te_BFS(n_u, n_v, r_u, r_v):

    # T1 includes v, T2 includes u
    if r_v.size < r_u.size:
        r_v = reconstruct_BFS_tree(n_v, set())
        return link(n_u, r_u, r_v)
    else:
        r_u = reconstruct_BFS_tree(n_u, set())
        return link(n_v, r_v, r_u)
...
def delete_te_simple(n_u, n_v):
    ...
    n_rs, n_rl = BFS_select_simple(r_s)
    ...
    if n_rs is None and n_rl is None:
        ...
        return insert_te_simple(n_rs, n_rl, r_s, r_l)
...
def delete_te(n_u, n_v):
    ...
    n_rs, n_rl, new_r = BFS_select(r_s)
    ...
    if n_rs is None and n_rl is None:
        ...
        return insert_te(n_rs, n_rl, r_s, r_l)
...
def reconstruct_BFS_tree(node, nodes_in_ST):
